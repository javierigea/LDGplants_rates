...
FISSE.binary.median <- function(phy, states, reps = 1000, tol=0.1, qratetype = "mk", incomplete = TRUE, ...){
  ...
  for (i in 1:reps){
    ...
    tset <- try(simTraitsParsimonyCriterion(phy, c(qq,qq), nc, tol, ...))
    if (class(tset) != "try-error"){
      mm[i,1] <- median(dx[names(tset)[tset == 0]])
      mm[i,2] <- median(dx[names(tset)[tset == 1]])			
    }else{
      obj <- list(lambda0 = median(lam0), lambda1 = median(lam1), pval = NA, null_mean_diff = NA, null_sd = NA, nchanges_parsimony = nc, qpars = qq)
      return(obj)	
    }
  ...
  delta <- mm[,2] - mm[,1]
  delta_true <- median(lam1) - median(lam0)
  pval <- sum(delta_true > delta) / (reps + 1)
  
  obj <- list(lambda0 = median(lam0), lambda1 = median(lam1), pval = pval, null_mean_diff = mean(delta), null_sd = sd(delta), nchanges_parsimony = nc, qpars = qq)
  return(obj)
}
