...
plot_Fig1_boxplot_troptempbinary<-function(){
  pdf('./plots/Fig1_boxplot_binary_lambda_log.pdf')
  blue<-brewer.pal(11,'RdYlBu')[9]
  red<-brewer.pal(11,'RdYlBu')[2]
...
plot_Fig1_quantilerate_vs_lambdarate<-function(){
...
plot_Fig1_strapp_samples_from_file<-function(correlationfile,parameter,tablefile){
  pdf('./plots/Fig1_strapp_correlations.pdf')
  par(pty='s')
...
plot_Fig1_strapp_density<-function(){
  ...
  GBIFdata.BAMM<-GBIFdata.BAMM[-which(duplicated(GBIFdata.BAMM$binomial)),]
  GBIFdata.BAMM<-unique(GBIFdata.BAMM)
  median.latitude<-abs(GBIFdata.BAMM$Median.Latitude)
  names(median.latitude)<-GBIFdata.BAMM$binomial
  cat('running STRAPP','\n')
  strapp.lambda<-traitDependentBAMM(BAMM.object, traits = median.latitude, 1000, rate = 'speciation', return.full = TRUE, method = 'spearman', logrates = TRUE, two.tailed = TRUE) 
  ...
  pdf('./plots/Fig1_strappdensity_full.pdf',paper='a4') 
  par(pty='s')
...
plot_Fig2_strapp_densities<-function(path.strappobjects){
  ...
  plot(c(0.1,0.1),type='n',xlim=c(-0.2,0.2),ylim=c(0,16),xaxs='i',yaxs='i',xlab='rho',ylab='',yaxt='n',bty='n')
