...
return_strapp_BAMM.object<-function(BAMM.object,trait.table,mode){
  if (mode=='continuous.abs.medianlatitude'){
    median.lat<-abs(trait.table$Median.Latitude)
    names(median.lat)<-trait.table$binomial
    cat('running strapp lambda','\n')
    lambda.median.lat<-traitDependentBAMM(BAMM.object, median.lat, 1000, rate = "speciation",return.full = TRUE, method = "spearman", logrates = TRUE,two.tailed = TRUE, traitorder = NA, nthreads = 1)
    return(lambda.median.lat)
  }
...
BAMM.object.subsampled<-readRDS(file=paste('./results/extremebias/all_clades_50million_1000samples_latitudedata_extremebias_',replicate,'.RDS',sep=''))
GBIFdata.BAMM.subsampled<-read.table(file=paste('./results/extremebias/input_tables/GBIFdata.BAMM.subsampled_extremebias_',replicate,'_table.txt',sep=''),sep='\t',header=T,stringsAsFactors=F)
abs.medianlatitude.strapp.object<-return_strapp_BAMM.object(BAMM.object = BAMM.object.subsampled,trait.table =GBIFdata.BAMM.subsampled,mode = 'continuous.abs.medianlatitude')
saveRDS(abs.medianlatitude.strapp.object,file=paste('./results/extremebias/abs.medianlatitude.strapp.extremebias_',replicate,'.RDS',sep=''))
